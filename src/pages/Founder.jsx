...
export default function Founder() {
  ...
  return (
    <>
      ...
      <PageHero tone="purple" sectionClassName="min-h-[680px] !pb-9 !pt-[112px] sm:!pb-10 sm:!pt-[124px] lg:!pb-[56px] lg:!pt-[120px] lg:min-h-[680px]" contentAlign="start" eyebrow="FOUNDER AND CEO" title="Leadership grounded in purpose and progress." lead="Sandeep Sandha guides the company with a practical focus on capable teams, reliable delivery and long-term client relationships." illustration="founder-momentum" crumbs={[{ label: 'Founder' }]} />
        ...
        <div className="container-x grid items-center gap-10 lg:grid-cols-12 lg:gap-16">
          <Reveal className="lg:col-span-5">
            <div tabIndex={0} role="button" aria-pressed={showCasualPortrait} aria-label="Sandeep Sandha portrait. Tap to switch between professional and casual portraits." onClick={() => setShowCasualPortrait((current) => !current)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); setShowCasualPortrait((current) => !current) } }} className="group relative mx-auto aspect-[4/5] max-w-[390px] cursor-pointer overflow-hidden rounded-[2rem] bg-gradient-to-br from-soft via-white to-softpurple shadow-lift focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-brand">
              ...
              <img src="/founder-sandeep-sandha-polo-smile.png" alt="" aria-hidden className={`absolute inset-0 size-full object-cover object-[center_22%] opacity-0 transition-[opacity,transform] duration-500 md:group-hover:scale-95 md:group-hover:opacity-100 md:group-focus-visible:scale-95 md:group-focus-visible:opacity-100 motion-reduce:transition-none ${showCasualPortrait ? 'scale-95 opacity-100' : ''}`} />
